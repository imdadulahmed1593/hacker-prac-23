...
Input.prototype.parser = function (testCases) {
  ...
  while (i < testCases.length) {
    ...
    dataObj.r = parseInt(testLine1[0]);
    dataObj.c = parseInt(testLine1[1]);
    dataObj.image = [];

    for (let j = i + 1; j <= i + dataObj.c; j++) {
      let imgInput = testCases[j].split('');
      dataObj.image.push(imgInput);
    }
